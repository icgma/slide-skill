...
def _render_guide(theme: ThemeSpec) -> str:
    ...
    lines = [
        ...
        "---",
        "",
        "## 4. Canvas & Optional Deck Motif",
        "",
        "Canvas: **1280 × 720 px**, always `width=\"1280\" height=\"720\" viewBox=\"0 0 1280 720\"`.",
        "",
        "### Optional deck motif (choose per deck; never required on every slide)",
        "",
        "A **motif** is a repeating visual device you may adopt ONCE per deck for",
        "consistency. Pick one if — and only if — it serves the deck's tone.",
        "Never apply a motif mechanically to every slide; slides whose content",
        "calls for a different composition should break away from it.",
        "",
        "Example motif — stripe + footer:",
        "```xml",
        '<g id="chrome-stripe">',
        ...
        '- [ ] Optional deck motif (if chosen) applied consistently — stripe `<rect x="0" y="0" width="6" height="720">` / footer `<rect x="0" y="688" width="1280" height="32">`',
