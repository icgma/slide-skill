...
- **Global scope**: `/tmp/slide-skill/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/tmp/slide-skill/.claude/notes/`)
...
2. Glob `/tmp/slide-skill/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/tmp/slide-skill/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/tmp/slide-skill/.claude/notes/` — works in any directory
